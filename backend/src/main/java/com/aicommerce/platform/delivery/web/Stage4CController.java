...
@ConditionalOnExpression("'${platform.adapter:}' == 'fake' && '${platform.web.enabled:false}' == 'true' && '${platform.stage4b.enabled:false}' == 'true' && '${platform.stage4c.enabled:false}' == 'true'")
@RequestMapping("/api")
public class Stage4CController {
    private final Stage4CService service;
    public Stage4CController(Stage4CService service){this.service=service;}

    @PostMapping(path="/platforms/meta/ad-sets/{adSetUuid}/ads/preview",consumes=MediaType.APPLICATION_JSON_VALUE)
    ...
    @PostMapping(path="/platforms/meta/ad-sets/{adSetUuid}/ads",consumes=MediaType.APPLICATION_JSON_VALUE)
    ...
    @GetMapping("/platforms/meta/ads/{adUuid}")
    ...
    @PostMapping(path="/platforms/meta/ads/{adUuid}/state/preview",consumes=MediaType.APPLICATION_JSON_VALUE)
    ...
    @PostMapping(path="/platforms/meta/ads/{adUuid}/{action:pause|resume}",consumes=MediaType.APPLICATION_JSON_VALUE)
    ...
    private ResponseEntity<Stage4BViews.Operation> response(Stage4BViews.Confirmation c){
        ...
        return ResponseEntity.status(status).eTag(ResourceEtag.format(v.version())).location(URI.create("/api/platform-operations/"+v.operationUuid())).body(v);
    }
