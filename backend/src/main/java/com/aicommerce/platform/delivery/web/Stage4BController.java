...
public class Stage4BController {
    ...
    private static void money(String field,String value){money(field,value,null);}
    private static void money(String field,String value,PlatformBudgetType type){
        if(value==null||!value.matches("^(?:0|[1-9][0-9]*)(?:\\.[0-9]{0,5}[1-9])?$"))invalid(field);
        try{
            var amount=new java.math.BigDecimal(value);
            var maximum=type==PlatformBudgetType.DAILY?new java.math.BigDecimal("100"):new java.math.BigDecimal("300");
            if(amount.signum()<=0||amount.scale()>6||amount.compareTo(maximum)>0)invalid(field);
