...
public class Stage4BTransactions {
    ...
    private final Environment environment; private final Stage4BLedgerCriticalSectionHook ledgerHook; private final Stage4BUuidSource uuids;
    public Stage4BTransactions(JdbcTemplate jdbc,PlatformOperationService operations,
            PlatformOperationJpaRepository operationRepository,ObjectMapper mapper,Environment environment,Stage4BLedgerCriticalSectionHook ledgerHook,Stage4BUuidSource uuids){
        this.jdbc=jdbc;this.operations=operations;this.operationRepository=operationRepository;this.mapper=mapper;this.environment=environment;this.ledgerHook=ledgerHook;this.uuids=uuids;
    }
    ...
    public Created confirmCampaign(UUID request,UUID campaignUuid,long expectedPlanVersion,String requestId){
        ...
        UUID entity=uuids.request(request,"campaign-entity"),operation=uuids.request(request,"operation");
    ...
    public Created confirmAdSet(UUID parent,UUID request,PlatformBudgetType type,BigDecimal amount,long expectedPlanVersion,long parentVersion,String requestId){
        ...
        validateBudget(type,amount,plan);UUID entity=uuids.request(request,"ad-set-entity"),operation=uuids.request(request,"operation");
        ...
        Day day=reserveDay(account,date,amount);insertReservation(request,batch,operation,account,day.uuid,entity,"INITIAL",null,amount,amount);applyDay(account,date,amount);
    ...
    public Created confirmState(PlatformEntityType type,UUID entity,UUID request,PlatformDesiredState target,long expectedVersion,String requestId){
        ...
        PlatformOperationType op=target==PlatformDesiredState.PAUSED?PlatformOperationType.PAUSE:PlatformOperationType.RESUME;UUID operation=uuids.request(request,"operation");
    ...
    public Created confirmBudget(UUID entity,UUID request,BigDecimal next,long expectedVersion,String requestId){
        ...
        BigDecimal delta=next.subtract(current.budget).max(BigDecimal.ZERO);String kind=delta.signum()>0?"INCREASE":"DECREASE_NO_RELEASE";UUID operation=uuids.request(request,"operation");
        ...
        insertReservation(request,batch,operation,account,day.uuid,entity,kind,current.budget,next,delta);applyDay(account,date,delta);
    ...
    private UUID insertBatch(UUID operation,UUID account,UUID request,Long expectedVersion,BigDecimal amount){UUID id=uuids.request(request,"operation-batch");jdbc.update("""
    ...
    private Day reserveDay(UUID account,LocalDate date,BigDecimal delta){UUID candidate=uuids.accountDay(account,date,"TWD");jdbc.update("""
    ...
    private void insertReservation(UUID request,UUID batch,UUID operation,UUID account,UUID day,UUID entity,String kind,BigDecimal previous,BigDecimal next,BigDecimal reserved){jdbc.update("""
      ...
      """,uuids.request(request,"budget-reservation"),batch,operation,account,day,entity,kind,previous,next,reserved);}

    UUID account(){return PlatformWebAccountLane.current()==PlatformWebAccountLane.GOOGLE?googleAccount():metaAccount();}
    private UUID metaAccount(){boolean test=Arrays.asList(environment.getActiveProfiles()).contains("test");UUID id=test?Stage4BAccountInitializer.TEST_UUID:Stage4BAccountInitializer.LOCAL_UUID;String reference=test?"stage4b-test":"stage4b-local";String expectedEnvironment=test?"TEST":"LOCAL";String fingerprint=test?"9276789d487fcd7791df964134173a1b815a4f9fc1d507457ee6dbcca187c8c2":"4f1eee978e5efed2d42ac62995484b642870cda74dea26cd2d2f63653d51cf36";return requireAccount("FAKE",id,reference,expectedEnvironment,fingerprint);}
    private UUID googleAccount(){boolean test=Arrays.asList(environment.getActiveProfiles()).contains("test");UUID id=test?Stage7C1AccountInitializer.TEST_UUID:Stage7C1AccountInitializer.LOCAL_UUID;String reference=test?"stage7c1-google-test":"stage7c1-google-local";String expectedEnvironment=test?"TEST":"LOCAL";String fingerprint=test?Stage7C1AccountInitializer.TEST_FINGERPRINT:Stage7C1AccountInitializer.LOCAL_FINGERPRINT;return requireAccount("FAKE_GOOGLE",id,reference,expectedEnvironment,fingerprint);}
    private UUID requireAccount(String providerKey,UUID id,String reference,String expectedEnvironment,String fingerprint){List<UUID> candidates=jdbc.query("SELECT platform_account_uuid FROM platform_accounts WHERE provider_key=? AND account_reference=?",(rs,n)->rs.getObject(1,UUID.class),providerKey,reference);if(candidates.size()!=1||!id.equals(candidates.getFirst()))throw error("PLATFORM_ACCOUNT_CONFIGURATION_INVALID",HttpStatus.SERVICE_UNAVAILABLE);Integer exact=jdbc.queryForObject("SELECT count(*) FROM platform_accounts WHERE platform_account_uuid=? AND provider_key=? AND environment=? AND account_reference=? AND external_account_fingerprint=? AND lifecycle_status='ACTIVE' AND archived_at IS NULL AND currency='TWD' AND timezone='Asia/Taipei'",Integer.class,id,providerKey,expectedEnvironment,reference,fingerprint);if(exact==null||exact!=1)throw error("PLATFORM_ACCOUNT_CONFIGURATION_INVALID",HttpStatus.SERVICE_UNAVAILABLE);return id;}
