...
public class Stage4BService {
    ...
    public Stage4BViews.Operation operation(UUID id){return view(operations.findById(id).orElseThrow(()->new Stage4BException("PLATFORM_RESOURCE_NOT_FOUND",HttpStatus.NOT_FOUND)));}
    public Stage4BViews.Confirmation retry(UUID id,long version){legacy(id);return new Stage4BViews.Confirmation(from(operationService.retry(id,version,java.time.Instant.now())),false);}
    public Stage4BViews.Confirmation reconcile(UUID id,long version){legacy(id);return new Stage4BViews.Confirmation(from(operationService.reconcile(id,version)),false);}
    ...
    private void legacy(UUID id){Integer count=jdbc.queryForObject("SELECT count(*) FROM platform_operation_batches WHERE operation_uuid=?",Integer.class,id);if(count==null||count!=1)throw new Stage4BException("PLATFORM_LEGACY_OPERATION_INERT",HttpStatus.CONFLICT);}
