-- Stage 07C-1 expands the FAKE provider-key allow-list. V1-V16 remain immutable.
DO $$
DECLARE
  cname text;
BEGIN
  ...
  WHERE nsp.nspname = current_schema()
    ...
    AND pg_get_constraintdef(con.oid) LIKE '%FAKE%'
    AND pg_get_constraintdef(con.oid) NOT LIKE '%FAKE_GOOGLE%'
    AND con.conname <> 'ck_platform_accounts_provider_key_v17';
  IF cname IS NULL THEN
    RAISE EXCEPTION 'platform_accounts provider_key FAKE check not found';
  END IF;
...
ALTER TABLE platform_accounts
  ADD CONSTRAINT ck_platform_accounts_provider_key_v17
  CHECK (provider_key IN ('FAKE', 'FAKE_GOOGLE'));
...
CREATE OR REPLACE FUNCTION is_valid_platform_evidence(value JSONB) RETURNS BOOLEAN LANGUAGE sql IMMUTABLE AS $$
 SELECT value IS NOT NULL AND jsonb_typeof(value)='object'
  ...
  AND value->>'providerKey' IN ('FAKE','FAKE_GOOGLE') AND value->>'attemptKind' IN ('SUBMIT','RECONCILE')
