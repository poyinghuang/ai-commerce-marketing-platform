...
CREATE UNIQUE INDEX uq_platform_operations_successful_create_entity ON platform_operations(platform_account_uuid,entity_type,COALESCE(platform_campaign_uuid,platform_ad_set_uuid,platform_ad_uuid)) WHERE status='SUCCEEDED' AND operation_type LIKE 'CREATE_%';
CREATE UNIQUE INDEX uq_platform_operations_successful_state_version ON platform_operations(platform_account_uuid,entity_type,COALESCE(platform_campaign_uuid,platform_ad_set_uuid,platform_ad_uuid),((request_payload->>'expectedEntityVersion')::bigint)) WHERE status='SUCCEEDED' AND operation_type IN ('PAUSE','RESUME');
...
BEGIN
 ...
 IF NEW.status IN ('SUBMITTING','RECONCILING') THEN NEW.claimed_at:=statement_timestamp(); END IF;
 IF OLD.status='FAILED_RETRYABLE' AND NEW.status='SUBMITTING' AND (OLD.next_attempt_at IS NULL OR statement_timestamp()<OLD.next_attempt_at) THEN RAISE EXCEPTION 'retry claim is not due' USING ERRCODE='23514'; END IF;
...
BEGIN
 ...
 ELSIF op.status<>'CREATED' THEN
   ...
   IF op.status='FAILED_RETRYABLE' AND (a.attempt_kind<>'SUBMIT' OR a.completed_at IS NULL OR NOT a.evidence?'retryAfterSeconds' OR op.next_attempt_at IS DISTINCT FROM a.completed_at+make_interval(secs=>(a.evidence->>'retryAfterSeconds')::integer)) THEN RAISE EXCEPTION 'retry schedule must equal attempt completion plus retry-after' USING ERRCODE='23514'; END IF;
