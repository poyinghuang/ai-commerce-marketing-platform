...
class PlatformBudgetAuditEventTest {
    ...
    @Test void rejectsMismatchedKindsSubjectsActionsAmountsCurrencyAndAggregateArithmetic() {
        ...
        for(var invalid:java.util.List.of(
                new Object[]{PlatformOperationType.CREATE_CAMPAIGN,PlatformEntityType.CAMPAIGN,PlatformReservationKind.INITIAL},
                new Object[]{PlatformOperationType.PAUSE,PlatformEntityType.AD_SET,PlatformReservationKind.INCREASE},
                new Object[]{PlatformOperationType.UPDATE_BUDGET,PlatformEntityType.AD_SET,PlatformReservationKind.DECREASE_NO_RELEASE})){
            var type=(PlatformOperationType)invalid[0];var entityType=(PlatformEntityType)invalid[1];var reservationKind=(PlatformReservationKind)invalid[2];
            assertThatThrownBy(()->new PlatformBudgetAuditEvent(PlatformAuditSubjectType.PLATFORM_ACCOUNT_BUDGET_DAY,day,AuditAction.UPDATE,PlatformBudgetAuditEventKind.ACCOUNT_DAY_RESERVED,operation,type,entityType,entity,batch,Optional.of(reservation),Optional.of(day),LocalDate.of(2026,1,1),Optional.of(reservationKind),"TWD",Optional.empty(),Optional.empty(),BigDecimal.ONE,Optional.of(BigDecimal.ZERO),Optional.of(BigDecimal.ONE))).isInstanceOf(IllegalArgumentException.class).hasMessage("PLATFORM_CONTRACT_INVALID");
        }
    }
