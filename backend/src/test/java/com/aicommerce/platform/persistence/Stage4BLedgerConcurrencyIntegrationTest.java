...
class Stage4BLedgerConcurrencyIntegrationTest {
    ...
    @Test void freshDayBelowCeilingCommitsBothSeparateParentTransactions() throws Exception {
        ...
        assertThat(result.first.code).isEqualTo("SUCCESS");assertThat(result.second.code).isEqualTo("SUCCESS");assertDayAndRows("500",2,2);assertThat(result.first.entity).isNotEqualTo(result.second.entity);assertSuccessGraph(result.first);assertSuccessGraph(result.second);
    }
    ...
    @Test void freshDayExactCeilingCommitsBothAfterIndependentBaseline() throws Exception {
        ...
        assertThat(results).extracting(Result::code).containsOnly("SUCCESS");assertDayAndRows("1000",4,4);assertThat(results).extracting(Result::entity).doesNotHaveDuplicates();results.forEach(this::assertSuccessGraph);
    }
    ...
    @Test void freshDayAboveCeilingHasDeterministicLoserAndCompleteRollback() throws Exception {
        ...
        assertThat(results.subList(0,3)).extracting(Result::code).containsOnly("SUCCESS");assertThat(loser.code).isEqualTo("PLATFORM_BUDGET_CAP_EXCEEDED");assertDayAndRows("900",3,3);results.subList(0,3).forEach(this::assertSuccessGraph);
        assertThat(jdbc.queryForObject("SELECT count(*) FROM platform_operations WHERE client_request_uuid=?",Integer.class,loser.request)).isZero();assertThat(jdbc.queryForObject("SELECT count(*) FROM platform_operation_batches WHERE client_request_uuid=?",Integer.class,loser.request)).isZero();assertThat(jdbc.queryForObject("SELECT count(*) FROM audit_logs WHERE request_id='ledger-worker-3'",Integer.class)).isZero();assertThat(jdbc.queryForObject("SELECT count(*) FROM audit_log_changes c JOIN audit_logs l ON l.audit_uuid=c.audit_uuid WHERE l.request_id='ledger-worker-3'",Integer.class)).isZero();
    ...
    private Result create(UUID campaign,UUID request,String amount,String requestId){Instant lower=dbNow();try{var value=transactions.confirmAdSet(campaign,request,PlatformBudgetType.LIFETIME,new BigDecimal(amount),0,0,requestId).operation();return new Result("SUCCESS",request,value.getEntityUuid(),requestId,lower,dbNow());}catch(Stage4BException failure){return new Result(failure.code(),request,null,requestId,lower,dbNow());}}
    private Result budget(UUID adSet,String amount,String requestId){UUID request=UUID.randomUUID();Instant lower=dbNow();try{var value=transactions.confirmBudget(adSet,request,new BigDecimal(amount),0,requestId).operation();return new Result("SUCCESS",request,value.getEntityUuid(),requestId,lower,dbNow());}catch(Stage4BException failure){return new Result(failure.code(),request,null,requestId,lower,dbNow());}}
    ...
    private void assertSuccessGraph(Result result){
        var operation=jdbc.queryForMap("SELECT operation_uuid,platform_account_uuid,operation_type,entity_type,coalesce(platform_campaign_uuid,platform_ad_set_uuid,platform_ad_uuid) entity_uuid,status,attempt_count,version,client_request_uuid,request_id FROM platform_operations WHERE client_request_uuid=?",result.request);
        UUID operationUuid=(UUID)operation.get("operation_uuid");assertThat(operation.get("entity_type")).isEqualTo("AD_SET");assertThat(operation.get("entity_uuid")).isEqualTo(result.entity);assertThat(operation.get("status")).isEqualTo("CREATED");assertThat(((Number)operation.get("attempt_count")).intValue()).isZero();assertThat(((Number)operation.get("version")).longValue()).isZero();assertThat(operation.get("request_id")).isEqualTo(result.requestId);
        var batch=jdbc.queryForMap("SELECT * FROM platform_operation_batches WHERE operation_uuid=?",operationUuid);UUID batchUuid=(UUID)batch.get("operation_batch_uuid");
        var reservation=jdbc.queryForMap("SELECT * FROM platform_budget_reservations WHERE operation_uuid=?",operationUuid);UUID reservationUuid=(UUID)reservation.get("budget_reservation_uuid"),dayUuid=(UUID)reservation.get("account_budget_day_uuid");
        var day=jdbc.queryForMap("SELECT * FROM platform_account_budget_days WHERE account_budget_day_uuid=?",dayUuid);
        assertThat(batch.get("platform_account_uuid")).isEqualTo(operation.get("platform_account_uuid"));assertThat(batch.get("client_request_uuid")).isEqualTo(result.request);assertThat(((Number)batch.get("version")).longValue()).isZero();
        assertThat(reservation.get("operation_batch_uuid")).isEqualTo(batchUuid);assertThat(reservation.get("platform_account_uuid")).isEqualTo(operation.get("platform_account_uuid"));assertThat(reservation.get("platform_ad_set_uuid")).isEqualTo(result.entity);assertThat(reservation.get("business_date")).isEqualTo(batch.get("business_date"));
        assertThat(day.get("platform_account_uuid")).isEqualTo(operation.get("platform_account_uuid"));assertThat(day.get("business_date")).isEqualTo(batch.get("business_date"));assertThat(day.get("currency")).isEqualTo("TWD");
        Instant batchTime=((java.sql.Timestamp)batch.get("created_at")).toInstant(),reservationTime=((java.sql.Timestamp)reservation.get("created_at")).toInstant();assertThat(batchTime).isBetween(result.lower,result.upper);assertThat(reservationTime).isBetween(result.lower,result.upper);
        ...
        var audits=jdbc.queryForList("SELECT stage4b_operation_ordinal,entity_type,entity_uuid,actor_type,actor_id,source,request_id FROM audit_logs WHERE operation_uuid=? ORDER BY stage4b_operation_ordinal",operationUuid);assertThat(audits).isNotEmpty();
        for(int i=0;i<audits.size();i++){var audit=audits.get(i);assertThat(((Number)audit.get("stage4b_operation_ordinal")).intValue()).isEqualTo(i);assertThat(audit.get("actor_type")).isEqualTo("LOCAL_ADMIN");assertThat(audit.get("actor_id")).isEqualTo("local-admin");assertThat(audit.get("source")).isEqualTo("API");assertThat(audit.get("request_id")).isEqualTo(result.requestId);String type=(String)audit.get("entity_type");UUID expected=switch(type){case "PLATFORM_OPERATION"->operationUuid;case "PLATFORM_OPERATION_BATCH"->batchUuid;case "PLATFORM_BUDGET_RESERVATION"->reservationUuid;case "PLATFORM_ACCOUNT_BUDGET_DAY"->dayUuid;case "PLATFORM_AD_SET"->result.entity;default->throw new AssertionError(type);};assertThat(audit.get("entity_uuid")).isEqualTo(expected);}
        assertThat(jdbc.queryForObject("SELECT count(*) FROM audit_log_changes c JOIN audit_logs l ON l.audit_uuid=c.audit_uuid WHERE l.operation_uuid=?",Integer.class,operationUuid)).isPositive();
    }
    ...
    private record Result(String code,UUID request,UUID entity,String requestId,Instant lower,Instant upper){} private record ConcurrentResult(Result first,Result second){}
}
