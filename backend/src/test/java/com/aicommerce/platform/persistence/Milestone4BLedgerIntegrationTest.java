...
class Milestone4BLedgerIntegrationTest {
    ...
    @Test void malformedBatchFirstInsertAndDeferredCommitMatrixRollsBackEntireGraph() {
        ...
        assertState("23514",()->tx(()->insertLegacyCreateAd(graph,UUID.randomUUID(),UUID.randomUUID()),false));assertThat(snapshot()).isEqualTo(pristine);
    ...
    @Test void legacyCreateAdInsertAfterV14IsRejectedByNewShapeTriggerNotLedger() {
        Graph graph=graph();String pristine=snapshot();
        assertStateInvariant("23514","expectedParentVersion",()->tx(()->insertLegacyCreateAd(graph,UUID.randomUUID(),UUID.randomUUID()),false));
        assertThat(snapshot()).isEqualTo(pristine);
    }
    ...
    private void insertUnapprovedOperation(Graph graph,UUID operation,UUID request){
        Map<String,Object> evidence=jdbc.queryForMap("SELECT product_uuid,asset_uuid,generation_output_uuid,review_decision_uuid,trim(approved_checksum_sha256) checksum FROM platform_ads WHERE platform_ad_uuid=?",graph.ad);
        long parent=jdbc.queryForObject("SELECT version FROM platform_ad_sets WHERE platform_ad_set_uuid=?",Long.class,graph.adSet);
        String payload="{\"schemaVersion\":1,\"operationType\":\"CREATE_AD\",\"entityType\":\"AD\",\"entityUuid\":\""+graph.ad+"\",\"platformAdUuid\":\""+graph.ad+"\",\"platformAdSetUuid\":\""+graph.adSet+"\",\"expectedParentVersion\":"+parent+",\"productUuid\":\""+evidence.get("product_uuid")+"\",\"assetUuid\":\""+evidence.get("asset_uuid")+"\",\"generationOutputUuid\":\""+evidence.get("generation_output_uuid")+"\",\"reviewDecisionUuid\":\""+evidence.get("review_decision_uuid")+"\",\"approvedChecksumSha256\":\""+evidence.get("checksum")+"\",\"creativeMappingKey\":\"APPROVED_IMAGE_ASSET_V1\",\"desiredState\":\"PAUSED\"}";
        ...
        jdbc.update("INSERT INTO platform_operations(operation_uuid,platform_account_uuid,operation_type,entity_type,platform_ad_uuid,client_request_uuid,idempotency_key,request_payload,request_sha256,requested_actor_type,requested_actor_id,request_id) VALUES (?,?,'CREATE_AD','AD',?,?,?,?::jsonb,?,'LOCAL_ADMIN','local-admin','direct-sql')",operation,graph.account,graph.ad,request,hex(operation),payload,sha);
    }
    private void insertLegacyCreateAd(Graph graph,UUID operation,UUID request){Map<String,Object> evidence=jdbc.queryForMap("SELECT product_uuid,asset_uuid,generation_output_uuid,review_decision_uuid,trim(approved_checksum_sha256) checksum FROM platform_ads WHERE platform_ad_uuid=?",graph.ad);String payload="{\"schemaVersion\":1,\"operationType\":\"CREATE_AD\",\"entityType\":\"AD\",\"entityUuid\":\""+graph.ad+"\",\"platformAdUuid\":\""+graph.ad+"\",\"platformAdSetUuid\":\""+graph.adSet+"\",\"productUuid\":\""+evidence.get("product_uuid")+"\",\"assetUuid\":\""+evidence.get("asset_uuid")+"\",\"generationOutputUuid\":\""+evidence.get("generation_output_uuid")+"\",\"reviewDecisionUuid\":\""+evidence.get("review_decision_uuid")+"\",\"approvedChecksumSha256\":\""+evidence.get("checksum")+"\",\"creativeMappingKey\":\"IMAGE_PRIMARY_V1\",\"desiredState\":\"PAUSED\"}";jdbc.update("INSERT INTO platform_operations(operation_uuid,platform_account_uuid,operation_type,entity_type,platform_ad_uuid,client_request_uuid,idempotency_key,request_payload,request_sha256,requested_actor_type,requested_actor_id,request_id) VALUES (?,?,'CREATE_AD','AD',?,?,?,?::jsonb,?,'LOCAL_ADMIN','local-admin','legacy-sql')",operation,graph.account,graph.ad,request,hex(operation),payload,hex(request));}
