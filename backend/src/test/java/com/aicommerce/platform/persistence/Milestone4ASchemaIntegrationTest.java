...
@Testcontainers @SpringBootTest @ActiveProfiles("test") class Milestone4ASchemaIntegrationTest {
 ...
 @Test void operationInputAndTerminalRowsCannotBeChangedOrDeleted(){
  ...
  UUID operation=insertOperation(account,campaign,payload);
 ...
 private UUID insertOperation(UUID account,UUID campaign,String payload){UUID operation=UUID.randomUUID(),request=UUID.randomUUID();new TransactionTemplate(transactionManager).executeWithoutResult(s->{jdbc.update("insert into platform_operation_batches(operation_batch_uuid,operation_uuid,platform_account_uuid,client_request_uuid,requested_actor_type,requested_actor_id,currency,business_date,reserved_amount,created_at,version) values (?,?,?,?,'LOCAL_ADMIN','strict-tester','TWD',platform_taipei_business_date(statement_timestamp()),0,statement_timestamp(),0)",UUID.randomUUID(),operation,account,request);jdbc.update("insert into platform_operations(operation_uuid,platform_account_uuid,operation_type,entity_type,platform_campaign_uuid,client_request_uuid,idempotency_key,request_payload,request_sha256,requested_actor_type,requested_actor_id,request_id,max_attempts) values (?,?, 'CREATE_CAMPAIGN','CAMPAIGN',?,?,?, ?::jsonb,?,'LOCAL_ADMIN','strict-tester',?,3)",operation,account,campaign,request,UUID.randomUUID().toString().replace("-","").repeat(2),payload,UUID.randomUUID().toString().replace("-","").repeat(2),"strict-"+operation);});return operation;}
