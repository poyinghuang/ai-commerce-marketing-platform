...
class CreativePlanCommandServiceTest {
 static final UUID PRODUCT=UUID.randomUUID(),PLAN=UUID.randomUUID(); CreativePlanJpaRepository plans=mock(CreativePlanJpaRepository.class);ProductJpaRepository products=mock(ProductJpaRepository.class);AuditOperationContextFactory contexts=mock(AuditOperationContextFactory.class);AuditWriter writer=mock(AuditWriter.class);com.aicommerce.platform.quality.application.ProductQualityRecalculationService quality=mock(com.aicommerce.platform.quality.application.ProductQualityRecalculationService.class);Clock clock=Clock.fixed(Instant.parse("2026-08-07T00:00:00Z"),ZoneOffset.UTC);CreativePlanCommandService service=new CreativePlanCommandService(plans,products,contexts,writer,new CreativePlanAuditChangeFactory(),quality,clock);
