...
透過統一 Platform Adapter 對接 Meta、Google、LINE、TikTok 等平台。Stage 04 已交付 Meta 的 FAKE LOCAL/TEST 切片。Stage 07 FAKE 已用同一組 ports 證明可擴充：第二個 FAKE Image / Storage bean，以及 `FAKE_GOOGLE` adapter（additive V17）與 gated `/platforms/google` UI。Live Google / LINE / TikTok、credential 與 spend 仍要人工核准。
