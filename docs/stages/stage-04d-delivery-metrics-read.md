...
Let `now` be PostgreSQL `statement_timestamp()` (HTTP uses the same clock source as other Stage 04 commands).

```text
taipeiDate = (now AT TIME ZONE 'Asia/Taipei')::date
windowStart = (taipeiDate - 1 day) at time zone 'Asia/Taipei'
windowEnd   =  taipeiDate         at time zone 'Asia/Taipei'
```

The stored interval is half-open `[windowStart, windowEnd)` with `windowEnd > windowStart`. Attribution is exactly `attribution_click_days=7` and `attribution_view_days=1`. Currency and timezone are copied from the locked account (`TWD`, `Asia/Taipei`) and must satisfy `verify_platform_metric_account_coherence()`.

Refresh always writes that canonical window. GET always selects that same window. Historical other windows that test fixtures may have inserted are not listed by Stage 4D.
...
Exact duplicate `source_fingerprint` for the identity is rejected by V12 uniqueness (`23505`). Application refresh maps that outcome to HTTP `200` of the existing matching latest row, inserts nothing, and does not bump entity version.

A changed observation creates revision `MAX(revision_number)+1` with strictly later `fetched_at`. Application code that receives a fake `fetchedAt` not strictly after the current latest `fetched_at` returns `PLATFORM_CONTRACT_INVALID` and inserts nothing.
...
| --- | --- | --- |
| `SUCCESS` (default) | `observedState` equals current desired `PAUSED` or `ACTIVE`; otherwise `UNKNOWN` | `FRESH` constants below |
| `DELAYED` | same observation rule | same constants, `freshnessStatus=DELAYED` |
| `UNAVAILABLE` | `UNKNOWN` | all base metrics empty, `FRESHNESS=UNAVAILABLE` |
...
3. Compute the canonical window from `statement_timestamp()`.
4. Commit/release the eligibility transaction. Call `PlatformMetricsReadPort.readWindow` with **no** open database transaction.
5. Persist transaction: lock `account -> entity` again; if the fingerprint equals the latest row, return that row as replay; if new, insert the next revision; commit.

Fingerprint canonical JSON is unchanged from Stage 4A: lowercase SHA-256 of UTF-8 JSON containing only `entityType`, the typed entity UUID, UTC `windowStart`/`windowEnd`, `timezone`, `attributionClickDays`, `attributionViewDays`, `currency`, nullable base metrics, and `freshnessStatus`. Object keys are lexically sorted; JSON null is retained; `revisionNumber`, `fetchedAt`, `createdAt`, derived metrics, and trace IDs are excluded.
...
Duplicate fingerprint `23505` maps to metrics `200` replay as specified.
...
| `POST /api/platform-entities/{entityType}/{entityUuid}/delivery-sync/preview` | empty body, no query | `200 DeliveryPreview` |
| `POST /api/platform-entities/{entityType}/{entityUuid}/delivery-sync` | empty body, no query | `200 DeliveryView` + entity weak `ETag` |
| `GET /api/platform-entities/{entityType}/{entityUuid}/metrics` | optional `asOf` only | `200 MetricsView` |
| `POST /api/platform-entities/{entityType}/{entityUuid}/metrics-refresh/preview` | empty body, no query | `200 MetricsPreview` |
| `POST /api/platform-entities/{entityType}/{entityUuid}/metrics-refresh` | empty body, no query | `200 MetricsView` |

`asOf` is exactly one RFC-3339 UTC instant matching `^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$`. Future `asOf`, empty, duplicated, extra query keys, fragments, or `asOf` on any non-metrics-GET route are `400 PLATFORM_REQUEST_INVALID` with field `query`. POST refresh/preview reject any query and any non-empty body.
...
- Latest and as-of selection, `present=false`, NULL bases, derived omit/compute, and duplicate-fingerprint refresh replay are executable.
